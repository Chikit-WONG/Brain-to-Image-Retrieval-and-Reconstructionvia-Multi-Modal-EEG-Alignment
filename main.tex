...
Before image generation, we apply SimpleAlignPipe to reduce the distribution gap between EEG-derived embeddings and image-space CLIP embeddings.
The aligned embedding is then used as the IP-Adapter conditioning signal for SDXL-Turbo.
...
\paragraph{Inference.}
The EEG-derived embedding is first processed by the downstream alignment module and then passed to IP-Adapter~\citep{ye2023ipadapter}, which conditions SDXL-Turbo to generate the final 224$\times$224 image.
...
\textbf{8-blur + EVNet, fixed (Ours)} & $\mathbf{85.3 \pm 0.8\%}$ & $\mathbf{98.5 \pm 0.4\%}$ & $\mathbf{88.9 \pm 1.1\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
The ``w/o SimpleAlignPipe'' setting directly uses EEG-derived embeddings for generation, while the full pipeline first aligns these embeddings to the image-embedding distribution before conditioning SDXL-Turbo through IP-Adapter.
...
Future work could explore cross-subject and multi-subject generalisation, higher-resolution EEG acquisition, image encoding approaches that better simulate human vision, and more powerful generative models.
...
\begin{itemize}[nosep]
  ...
  \item Haowen YAN: Participated in the coding work for Task 2, conducted reconstruction experiments and evaluation, prepared qualitative examples, and contributed to the final report and presentation.
\end{itemize}
