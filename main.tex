...
\paragraph{Dataset.}
The provided preprocessed THINGS-EEG2 files contain repeated EEG responses for each stimulus.
We use preprocessed EEG data from a single subject with 63 channels sampled at 250\,Hz.
Each response is represented by a 250-sample post-stimulus segment, corresponding to approximately 1 second.
For both training and testing, we average over the repetition dimension of the provided tensors, yielding 16,540 training EEG-image pairs and 200 held-out test samples under the official 200-way protocol.
...
Given an EEG segment $\mathbf{e}$, generate an image that is semantically and perceptually consistent with the viewed stimulus.
Performance is measured by CLIP Score~\citep{radford2021clip} and additional feature-based reconstruction metrics computed by the provided evaluation script: SSIM~\citep{wang2004ssim}, AlexNet- and Inception-based two-way identification scores, and EfficientNet correlation distance.
...
\end{enumerate}
The EEG encoder outputs a 1024-dimensional embedding $\mathbf{z}_\text{EEG} \in \mathbb{R}^{1024}$.
...
The 8 feature vectors are first aggregated via a learned attention mechanism (softmax-weighted sum) into a single 1024-dimensional blur feature.
...
The EVNet output is subsequently processed by the same frozen CLIP RN50 backbone, yielding a complementary 1024-dimensional feature.

\paragraph{Stream Fusion.}
When EVNet is enabled, the aggregated blur feature and the EVNet feature are combined via a learned softmax gate:
\begin{equation}
  \mathbf{v}_\text{fused} = w_1 \cdot \mathbf{v}_\text{blur} + w_2 \cdot \mathbf{v}_\text{EVNet}, \quad (w_1, w_2) = \text{softmax}(\boldsymbol{\alpha})
\end{equation}
where $\boldsymbol{\alpha} \in \mathbb{R}^2$ is a learnable parameter.
The fused feature is then passed through a shared two-layer fusion adapter (1024 $\to$ 768 $\to$ 1024, dropout $=0.85$), producing the final image embedding $\mathbf{v} \in \mathbb{R}^{1024}$.
If EVNet is disabled, the aggregated blur feature is passed through the corresponding blur-only adapter instead.
...
\begin{equation}
  \mathcal{L} = -\frac{1}{2N}\sum_{i=1}^N \left[\log\frac{e^{\mathbf{z}_i \cdot \mathbf{v}_i}}{\sum_j e^{\mathbf{z}_i \cdot \mathbf{v}_j}} + \log\frac{e^{\mathbf{v}_i \cdot \mathbf{z}_i}}{\sum_j e^{\mathbf{v}_i \cdot \mathbf{z}_j}}\right]
\end{equation}
In our implementation, the logits use a fixed scale of $1$ rather than a learned temperature.
At retrieval time, both EEG and visual embeddings are $\ell_2$-normalized before cosine-similarity ranking over the 200 candidate images.

\begin{figure}
  \centering
  \includegraphics[width=1.0\linewidth]{images/Task1.png}
...
% \begin{figure}
%     \centering
%     \includegraphics[width=0.9\linewidth]{images/Task2_v2.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.9\linewidth]{images/Task2_v3.png}
    \caption{Pipeline of the Training Process of Image Reconstruction.}
    \label{fig:task2_pipeline}
\end{figure}
...
Each single-modality branch is trained with the Similarity-Category Masked Loss (SCM-Loss) described in CogCapPro~\citep{ccp2026}.
Following the naming convention in the official CogCapPro implementation, our code implements this objective as \texttt{ClipLoss\_Modified\_DDP}, a modified CLIP loss with similarity-based top-$k$ masking and class-based masking.
...
\paragraph{Inference.}
The EEG-derived embeddings are first processed by the downstream alignment module, and the aligned image, depth, and edge embeddings are then passed to IP-Adapter~\citep{ye2023ipadapter}, which conditions SDXL-Turbo to generate the final reconstructed image.
Generated images are resized as required by each evaluation metric during scoring.
...
The provided tensors contain repeated EEG responses per stimulus; we average across the repetition dimension for both training and testing, resulting in 16,540 training samples and 200 held-out samples.
...
\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=1pt]
  ...
  \item Time window: 250 post-stimulus samples (approximately 1 second at 250\,Hz)
...
Task~2: CLIP Score, SSIM~\citep{wang2004ssim}, AlexNet-layer two-way identification, Inception-based two-way identification, and EfficientNet correlation distance, all computed by the provided evaluation script.
...
Table~\ref{tab:task1} summarises the primary Task~1 result from the \textbf{8-blur + EVNet full-train} configuration, averaged across 10 seeds (seeds 21--30) for subject sub-01, using 200 epochs, batch size 1024, and learning rate $10^{-3}$.
...
\begin{table}[H]
\centering
\caption{Task 1 retrieval accuracy for the 8-blur + EVNet full-train configuration (200-way, mean $\pm$ std, 10 seeds, subject sub-01, 200 epochs, batch size 1024, learning rate $10^{-3}$). \textdagger~Best-epoch is reported for reference only and is \emph{not} used as the leaderboard metric.}
\label{tab:task1}
\begin{tabular}{lcc}
\toprule
...
AlexNet-2 two-way ID ($\uparrow$)    & $0.762 \pm 0.012$ \\
AlexNet-5 two-way ID ($\uparrow$)    & $0.883 \pm 0.009$ \\
Inception two-way ID ($\uparrow$)    & $0.817 \pm 0.008$ \\
EfficientNet corr.\ dist.\ ($\downarrow$) & $0.828 \pm 0.004$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Task 1 ablation: effect of blur levels and EVNet (95/5 split, mean $\pm$ std, 10 seeds, 200-way). The last two rows compare the 8-blur baseline against the matched fixed 8-blur + EVNet setting used for ablation comparison. \textdagger~Best-test shown for reference only.}
\label{tab:ablation_task1}
\setlength{\tabcolsep}{4pt}
\small
\begin{tabular}{lccc}
\toprule
\textbf{Configuration} & \textbf{Val-sel Top-1} & \textbf{Final Epoch Top-1} & \textbf{Best-test Top-1\textdagger} \\
\midrule
No blur, no EVNet (Baseline)       & $61.2 \pm 2.4\%$ & $64.3 \pm 1.8\%$ & $67.1 \pm 1.0\%$ \\
EVNet, no blur                     & $70.8 \pm 2.1\%$ & $71.7 \pm 2.1\%$ & $75.7 \pm 0.8\%$ \\
8-blur                            & $82.8 \pm 2.3\%$ & $84.0 \pm 1.4\%$ & $87.1 \pm 0.9\%$ \\
\textbf{8-blur + EVNet (fixed setting)} & $\mathbf{85.3 \pm 0.8\%}$ & $\mathbf{85.9 \pm 2.7\%}$ & $\mathbf{88.9 \pm 1.1\%}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[H]
\centering
\caption{Task 1 ablation: EVNet initialisation and backbone (8-blur + EVNet, 95/5 split, mean $\pm$ std, 10 seeds). \textdagger~Best-test shown for reference only.}
...
\textbf{Configuration} & \textbf{Val-sel Top-1} & \textbf{Final Epoch Top-1} & \textbf{Best-test Top-1\textdagger} \\
\midrule
\textbf{RN50 + EVNet, Kaiming init (Ours)} & $\mathbf{84.6 \pm 1.3\%}$ & $\mathbf{84.8 \pm 1.7\%}$ & $\mathbf{87.2 \pm 0.9\%}$ \\
RN50 + EVNet, Xavier init   & $82.8 \pm 1.7\%$ & $81.9 \pm 1.3\%$ & $85.0 \pm 0.8\%$ \\
RN50 + GAP (no CLIP backbone) & $82.9 \pm 1.6\%$ & $83.2 \pm 1.6\%$ & $86.2 \pm 0.9\%$ \\
ViT-H/14 + EVNet (replaces RN50) & $73.7 \pm 2.0\%$ & $73.6 \pm 2.0\%$ & $77.9 \pm 1.1\%$ \\
\bottomrule
\end{tabular}
\end{table}
...
AlexNet-2 two-way ID ($\uparrow$)   & $0.657 \pm 0.007$ & $\mathbf{0.762 \pm 0.012}$ \\
AlexNet-5 two-way ID ($\uparrow$)   & $0.698 \pm 0.016$ & $\mathbf{0.883 \pm 0.009}$ \\
Inception two-way ID ($\uparrow$)   & $0.598 \pm 0.009$ & $\mathbf{0.817 \pm 0.008}$ \\
EfficientNet corr.\ dist.\ ($\downarrow$) & $0.952 \pm 0.006$ & $\mathbf{0.828 \pm 0.004}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=2pt]
  \item \textbf{Aircraft carrier} (\#1): The reconstruction captures the tower-like structure, reflecting low-frequency spatial encoding from the multi-modal stream.
...
Results in Table~\ref{tab:ablation_task1} show that adding EVNet consistently improves over the corresponding no-EVNet settings, while the 8-blur and 12-blur EVNet variants remain close.
This supports the view that the blur hierarchy and EVNet stream provide complementary information about visual stimuli, while also justifying our choice of the more compact 8-blur setting for the final full-train run in Table~\ref{tab:task1}.
...
Without this alignment stage, the EEG-derived image, depth, and edge embeddings remain poorly matched to the corresponding IP-Adapter embedding distributions expected by the generation pipeline, producing reconstructions with low CLIP Score (0.694) and weaker perceptual quality.
With SimpleAlignPipe, CLIP Score improves by $+0.199$ ($+28.7\%$) and SSIM improves by $+0.057$ ($+19.0\%$).
Under the EfficientNet correlation-distance metric, the aligned pipeline also improves over the unaligned baseline (0.952 $\to$ 0.828; lower is better), indicating better feature-level consistency under that metric as well.
