...
\large DSAA2012 Deep Learning --- Final Project Report}
...
For reconstruction (Task~2), we implemented CognitiveCapturerPro (CogCapPro)~\citep{ccp2026} that aligns EEG representations to multi-modal CLIP embeddings (image, text, depth, edge) and synthesizes images with SDXL-Turbo~\citep{sauer2023sdxlturbo} conditioned via IP-Adapter~\citep{ye2023ipadapter}.
...
In this project, we tackle two complementary tasks using preprocessed THINGS-EEG2 dataset~\citep{gifford2022thingseeg2data}. We would use blurring method of~\citealp{ved2026}, improved with EVNet~\citep{evnet2025} front end to preprocess images in the retrieval task, and use CognitiveCapturerPro (CogCapPro)~\citep{ccp2026} in the reconstruction task.
...
We use a preprocessed EEG data from a single subject.
Each EEG trial spans a 0--250\,ms post-stimulus window (250 time samples), averaged across the 4 trials to suppress noise.
The dataset contains 16,540 training samples (1,654 classes with 10 samples in each class) and 200 held-out test samples (200 classes with 1 sample in each class). Each sample contains 4 trials of EEG data and one corresponding ${224 \times 224}$ RGB image.
...
Rather than using a single image representation, or using 10 blurring levels described by~\citealp{ved2026}, we apply 8 levels of Gaussian blur under convolution kernel size $\in \{1, 3, 15, 21, 33, 45, 57, 63\}$ pixels to each training image and extract features from each blurred version using a frozen CLIP RN50 encoder~\citep{radford2021clip} (as also studied in~\citep{cherti2023openclip}).
...
\begin{figure}
  ...
  \caption{Image Retrieval Training Pipeline. Green components indicate fixed processes, while red components indicate processes that contains trainable weights.}
...
A dedicated \texttt{EEGProjectLayer} maps the EEG signal to four parallel embedding spaces corresponding to four  modalities: \emph{image} (CLIP ViT-H-14), \emph{text} (CLIP ViT-H-14 text encoder), \emph{depth map}, and \emph{edge map}. 
...
Each modality head consists of fully-connected layers with residual connections, producing a latent vector $\mathbf{z}_m \in \mathbb{R}^{d}$ for modality $m$.

\paragraph{Multi-Modal Alignment.}
Each modality head is trained with SCM-Loss~\citep{ccp2026} against frozen CLIP targets. A fusion encoder integrates all four modalities via cross-modal attention, followed by a shared-trunk alignment module that maps them into a unified image embedding space.

\paragraph{Image Generation.}
At inference time, the predicted image-modality embedding $\hat{\mathbf{z}}_\text{img}$ is used to condition SDXL-Turbo~\citep{sauer2023sdxlturbo,podell2023sdxl} via an IP-Adapter~\citep{ye2023ipadapter}, which injects visual semantic information into the cross-attention layers of the U-Net denoiser.
...
\begin{enumerate}[leftmargin=*, topsep=2pt, itemsep=1pt]
  \item \textbf{Stage 1 (epochs 0--20):} Single-modality contrastive training for each head independently with batch size 1024.
  \item \textbf{Stage 2 (epochs 20--60):} Joint multi-modal training; all four modality heads trained simultaneously.
  \item \textbf{Stage 3 (epochs 60--80):} Fusion-only training; the fusion head is fine-tuned on concatenated multi-modal embeddings.
\end{enumerate}
We apply \emph{uncertainty-aware} weighting to de-emphasise noisy EEG trials, and \emph{trial averaging} (averaging 4 test trial repetitions) to improve signal quality.

\paragraph{Inference.}
The predicted embedding is passed to IP-Adapter~\citep{ye2023ipadapter}, which conditions SDXL-Turbo to generate the final 224$\times$224 image.
...
Training and testing uses trial-averaged EEG signals to 16,540 training samples and 200 held-out samples.
...
\paragraph{Task 2: Effect of Contrastive Alignment.}
Table~\ref{tab:ablation_task2} compares reconstruction quality with and without the multi-modal contrastive alignment training (CogCapPro).
``w/o alignment'' generates images directly from raw (unaligned) EEG projections passed to IP-Adapter, bypassing the contrastive training stage.
``Full pipeline'' uses the complete three-stage alignment procedure before generation.

\begin{table}[H]
\centering
\caption{Task 2 ablation: contrastive alignment vs.\ no alignment (mean $\pm$ std, 5 seeds).}
\label{tab:ablation_task2}
\setlength{\tabcolsep}{4pt}
\begin{tabular}{lcc}
\toprule
\textbf{Metric} & \textbf{w/o Alignment} & \textbf{Full Pipeline} \\
\midrule
...
\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=2pt]
  ...
  \item \textbf{Orchid} (\#9): The approach fails in reconstructing this floral stimuli with complex colour patterns.
...
Overall, the model performs better on categories with strong, distinctive visual features (colour, shape) objects.
...
The ablation in Table~\ref{tab:ablation_task1} quantifies the individual contribution of multi-blur and EVNet components. However, this approach in Task 1 does not help in reconstruction tasks (Task 2), which makes the architecture of Task 1 differ from Task 2.   
...
\paragraph{Importance of contrastive alignment (Task 2 ablation).}
Table~\ref{tab:ablation_task2} shows that contrastive alignment is the decisive factor in Task~2.
Without alignment, the IP-Adapter receives an arbitrary EEG projection that has no semantic correspondence to CLIP space, producing reconstructions with low CLIP Score (0.694) and poor perceptual quality.
The full alignment pipeline raises CLIP Score by $+0.199$ ($+28.7\%$) and SSIM by $+0.057$ ($+19.0\%$).
...
\paragraph{Staged training (Task 2).}
The three-stage training curriculum (single $\to$ joint $\to$ fusion) prevents gradient conflicts between modalities in early training and allows each modality head to develop a useful initialisation before cross-modal interactions are introduced.

\paragraph{Limitations.}
The primary limitations are: (1) preprocessing (blurring and EVNet) approach in Task 1 does not help in reconstruction tasks; (2) the model is trained on a single subject, limiting generalisability; (3) reconstruction quality for fine-grained or abstract categories remains limited; (4) the evaluation pool of 200 classes, while standard, is relatively small.
...
\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=2pt]
  ...
  \item \textbf{CognitiveCapturerPro}~\citep{ccp2026}: Used as the general code backbone in Task~2.
...
No test-set labels or additional data beyond the provided training set were used.
...
For reconstruction, CognitiveCapturePro's multi-modal alignment pipeline paired with SDXL-Turbo generation achieves a CLIP Score of 0.893 and SSIM of 0.356, substantially better than the unaligned baseline.
These results demonstrate the feasibility of decoding rich visual representations from EEG signals using modern deep learning techniques.
Future work could explore cross-subject and multi-subject generalisation, higher-resolution EEG acquisition, image encoding approaches that better simulates human visions, and more powerful generative models.
